...
def clips_summary(conn):
	...
	return clips.groupby('species_name').agg({'species_id': 'count', 
                         'how_many':'sum'})

def get_species_frames(species_name, conn):

	# Get species_id from name
	species_id = pd.read_sql_query(f"SELECT id FROM species WHERE label='{species_name}'", conn).values[0][0]

	# Get clips for species from db
	clips = [i[0] for i in pd.read_sql_query(f"SELECT clip_id FROM agg_annotations_clip WHERE species_id={species_id}", conn).values]

	# Get filenames of clips
	filenames = [i[0] for i in pd.read_sql_query(f"SELECT filename FROM clips WHERE id IN {tuple(clips)}", conn).values]
