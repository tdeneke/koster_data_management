...
def get_movies_id(df, db_path):
    ...
    df = pd.merge(
        df, movies_df, how="left", on="movie_filename"
    )
...
def process_manual_clips(meta_df, db_path):
    ...
    meta_df["movie_filename"] = meta_df["movie_filename_ext"].str.replace(
        ".mp4", ""
    )
...
def main():
    ...
    man_clips_df = clips_df[clips_df["created_at"].between(first_manual_clips,last_manual_clips)]
    ...
    man_frames_df = frames_df[frames_df["created_at"].between(first_manual_frames,last_manual_frames)]
    ...
    man_frames_meta = man_frames_meta.rename(
        columns={"movie_frame": "frame_number"}
    )
